...
v0.2.2将正式分支统一到main，并固定含MinerU云端与Web凭据配置的插件SHA；仅更新发行版本与说明，不修改Harness实现。CI 将已测试镜像原样推送 GHCR，运行记录和digest随Release发布；日常实例不会自动升级。源码pin与镜像验收范围由 [UPSTREAM_BASE](UPSTREAM_BASE.md) 说明。
