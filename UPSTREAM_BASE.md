...
历史实测镜像与此前插件`a0fa0c6a851cb106ac00b02b3381b5c126110837`的184个运行文件等价，旧等价证明见plugin artifacts/v02-runtime-equivalence.json；docker-result.json为可覆盖的当前验收回执，必须核对SHA。本次新pin包含数学显示、PDF缓存代激活/默认模式/上传deadline、plan卡片修复和新版演示；它不等价于旧镜像。本轮68ba236镜像build/health通过，Chromium暴露上述任务会话竞态；修复后的packed浏览器已通过。v0.2.1 的 tag-only Release CI 会独立构建当前pin，并验收冷启动、完整Chromium与重启后再推送GHCR；实际结果由同名Release的docker-result.json和image-digest.txt拥有，不继承旧镜像PASS，也不自动替换日常容器。
